...
export class ProjectRepository {
  ...
  async findAll(options: ProjectListOptions): Promise<{ data: ProjectWithNames[]; page: { nextCursor: string | null; hasMore: boolean } }> {
    const limit = Math.min(Math.max(options.limit ?? 20, 1), 100);
    const rows = await this.db.select(projectColumns).from(projects)
      ...
      .where(and(
        ...
        options.cursor ? lt(projects.createdAt, new Date(options.cursor)) : undefined,
      )).orderBy(desc(projects.createdAt)).limit(limit + 1);
  ...
  async findById(id: string): Promise<ProjectWithNames | undefined> {
    const [project] = await this.db.select(projectColumns).from(projects)
      .innerJoin(customers, eq(projects.customerId, customers.id))
      .innerJoin(users, eq(projects.ownerId, users.id))
  ...
  async create(data: NewProject): Promise<ProjectWithNames> {
    const [project] = await this.db.insert(projects).values(data).returning({ id: projects.id });
  ...
  async update(id: string, data: Partial<Omit<NewProject, 'id' | 'createdAt' | 'updatedAt' | 'revision'>>, expectedRevision: number): Promise<ProjectWithNames | null> {
    const [project] = await this.db.update(projects)
      .set({ ...data, updatedAt: new Date(), revision: sql`${projects.revision} + 1` })
  ...
  async findMembers(projectId: string): Promise<ProjectMemberWithUser[]> {
    return this.db.select({ userId: projectMembers.userId, userName: users.displayName, userEmail: users.email, userRole: users.role, addedBy: projectMembers.addedBy, addedAt: projectMembers.addedAt })
      .from(projectMembers).innerJoin(users, eq(projectMembers.userId, users.id))
      .where(eq(projectMembers.projectId, projectId)).orderBy(users.displayName);
  }

  async addMember(projectId: string, userId: string, addedBy: string): Promise<boolean> {
    const rows = await this.db.insert(projectMembers).values({ projectId, userId, addedBy }).onConflictDoNothing().returning({ userId: projectMembers.userId });
  ...
  async updateOwner(projectId: string, newOwnerId: string, expectedRevision: number): Promise<ProjectWithNames | null> {
