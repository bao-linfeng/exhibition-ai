...
import { FolderKanban, Calendar, MapPin, User, Users, ChevronRight, Plus, Trash2 } from '@lucide/vue';
...
const { data: membersData, isLoading: isLoadingMembers } = useProjectMembersQuery(projectId.value);
...
const availableUsers = computed(() => {
  ...
  const memberIds = membersData.value.map((m: any) => m.userId);
  return usersData.value.data.filter((u: any) => !memberIds.includes(u.id));
});
</script>
...
<template>
  ...
  <div v-else-if="project">
    <div class="mb-4 flex items-center text-sm text-muted-foreground">
      <button @click="goBack" class="hover:text-foreground transition-colors">项目管理</button>
        ...
        <div class="rounded-xl border bg-card text-card-foreground shadow-sm overflow-hidden">
          <div class="p-6 border-b bg-muted/20">
            ...
            <div class="mt-2 text-sm">
              ...
              <button @click="viewCustomer" class="text-primary hover:underline font-medium">
          ...
          <div class="p-6 space-y-4">
            <div class="flex items-start gap-3">
              <div class="mt-0.5 text-muted-foreground"><User class="h-4 w-4" /></div>
              <div>
                <p class="text-sm font-medium">负责人</p>
                <p class="text-sm text-muted-foreground">{{ project.ownerName }}</p>
              </div>
            </div>
            
            <div v-if="project.exhibitionName" class="flex items-start gap-3">
              <div class="mt-0.5 text-muted-foreground"><MapPin class="h-4 w-4" /></div>
              <div>
                <p class="text-sm font-medium">展会名称</p>
                <p class="text-sm text-muted-foreground">{{ project.exhibitionName }}</p>
                <p v-if="project.exhibitionVenue" class="text-sm text-muted-foreground mt-0.5">
                  {{ project.exhibitionVenue }} <span v-if="project.boothNumber">| {{ project.boothNumber }}</span>
                </p>
              </div>
            </div>
            
            <div v-if="project.exhibitionDate" class="flex items-start gap-3">
              <div class="mt-0.5 text-muted-foreground"><Calendar class="h-4 w-4" /></div>
              <div>
                <p class="text-sm font-medium">展会日期</p>
                <p class="text-sm text-muted-foreground">{{ project.exhibitionDate }}</p>
              </div>
            </div>
            
            <div v-if="project.deliveryDeadline" class="flex items-start gap-3">
              <div class="mt-0.5 text-muted-foreground"><Calendar class="h-4 w-4 text-destructive" /></div>
              <div>
                <p class="text-sm font-medium">交付截止日期</p>
                <p class="text-sm text-destructive font-medium">{{ project.deliveryDeadline }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="p-0">
            ...
            <Table v-else>
              ...
              <TableBody>
                ...
                <TableRow v-for="member in membersData || []" :key="member.userId">
                  <TableCell class="font-medium">
                    {{ member.userName }}
                    <span v-if="member.userId === project.ownerId" class="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">负责人</span>
                  </TableCell>
                  <TableCell>{{ member.userEmail }}</TableCell>
                  <TableCell>
                    <span class="text-xs border px-2 py-1 rounded-full">{{ member.userRole }}</span>
                  </TableCell>
    ...
    <Dialog :open="showAddMemberDialog" @update:open="showAddMemberDialog = $event">
      <DialogContent class="sm:max-w-[425px]">
        ...
        <DialogFooter>
          <Button variant="outline" @click="showAddMemberDialog = false">取消</Button>
          <Button type="button" @click="addMember" :disabled="!selectedUserId || addMemberMutation.isPending.value">
